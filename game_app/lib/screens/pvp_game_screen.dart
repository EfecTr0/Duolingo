...
import '../main.dart' show startSpeechRecognition, stopSpeechRecognition,
    switchBackgroundMusic, playFinishSound, playClickSound;
...
class _PvPGameScreenState extends State<PvPGameScreen> with TickerProviderStateMixin {
  ...
  void dispose() {
    ...
    ApiService.leaveGame(widget.sessionId);
  ...
  Widget _buildResultScreen() {
    // Наши ответы и правильные ответы
    int playerCorrect = 0;
    int totalQuestions = 0;
    for (final round in _roundsHistory) {
      totalQuestions += round.opponentWords.length;
      for (int i = 0; i < round.opponentWords.length; i++) {
        if (i < round.playerAnswers.length) {
          final correctRussian = _getRussianTranslation(round.opponentWords[i]);
          if (correctRussian != null &&
              round.playerAnswers[i].trim().toLowerCase() == correctRussian.toLowerCase()) {
            playerCorrect++;
          }
        }
      }
    ...
    // Ответы соперника – получаем из gameData
    final opponentAnswersRaw = (_gameData!['opponent_answers'] as List).cast<List>();
    final ourSelected = List<String>.from(_gameData!['selected'] ?? []);
    int opponentCorrect = 0;
    int opponentTotal = 0;
    for (int r = 0; r < opponentAnswersRaw.length; r++) {
      final answers = List<String>.from(opponentAnswersRaw[r]);
      for (int i = 0; i < answers.length; i++) {
        if (i < ourSelected.length) {
          final correctRussian = _getRussianTranslation(ourSelected[i]);
          opponentTotal++;
          if (correctRussian != null &&
              answers[i].trim().toLowerCase() == correctRussian.toLowerCase()) {
            opponentCorrect++;
          }
        }
      }
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text('Правильных: $opponentCorrect / $opponentTotal'),
  ...
  String? _getRussianTranslation(String english) {
    for (final w in allWords) {
      if (w.english == english) return w.russian;
    }
    return null;
  }
}

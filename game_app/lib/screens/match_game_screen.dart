...
class _MatchGameScreenState extends State<MatchGameScreen> {
  ...
  void _onRussianTap(String russian) {
    ...
    if (selectedWord.russian == russian) {
      ...
      if (_attemptsLeftInSet <= 0) {
        // Оставшиеся слова помечаем как 'unanswered' (уже по умолчанию)
        _nextSet();
      }
      setState(() {});
    }
  ...
  void _nextSet() {
    // Оставшиеся слова, которые не были ни правильно, ни неправильно, остаются 'unanswered'
  ...
  void _showResultDialog() {
    ...
    // Собираем все пары из всех раундов для отображения (пришлось бы хранить всю историю, но для простоты покажем только последний раунд,
    // т.к. полная история не сохраняется. Сделаем заглушку: выведем список всех слов из _initialWords последнего раунда.
    // В реальности нужно хранить все раунды, но для демонстрации оставим только последний набор.
    // Показываем все слова из последнего раунда с их статусом.

...
class ApiService {
  ...
  // PvP
  static Future<Map<String, dynamic>> sendInvite(int friendId) async {
    final token = await getToken();
    if (token == null) throw Exception('Not authenticated');
    final response = await http.post(Uri.parse('$baseUrl/invite'), headers: {'Content-Type': 'application/json', 'X-Auth-Token': token}, body: jsonEncode({'friend_id': friendId}));
    if (response.statusCode == 200) return jsonDecode(response.body);
